...
CONFIG: dict = {
    ...
    "leverage":         10,         # Плечо фьючерсов (10x)
    ...
    "position_risk_check_interval": 60,  # Проверка риска позиций каждые 60 сек
    "max_wait_cycles": 10,             # Макс циклов ожидания закрытия (10 * 10сек = 100сек)
    "risk_close_threshold": 0.02,      # Если риск > 2% от баланса — закрыть немедленно
